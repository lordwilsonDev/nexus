...
every experiment lives in its own standalone runner; this file runs them and
aggregates the verdicts into a factory-style weakest-verdict gate. The hermes
factory invokes this with `--all --json`.

Experiments (serverless — no daemon required, no auth gate to probe):
  n01_config_load    config.yaml loads into NexusConfig without error
  n02_intent_router  IntentRouter(use_llm=False) is deterministic and routes
                     known keyword intents correctly
  n03_import_surface every top-level module imports cleanly

Usage:
    python hygiene_runner.py --all                 # run every experiment
    python hygiene_runner.py n01                   # run one experiment
    python hygiene_runner.py --only n01 n03        # run a subset
...
EXPERIMENTS: dict[str, Path] = {
    ...
    "n02_intent_router": HERE / "n02_intent_router_runner.py",
    "n03_import_surface": HERE / "n03_import_surface_runner.py",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NEXUS hygiene runner (delegating index)")
